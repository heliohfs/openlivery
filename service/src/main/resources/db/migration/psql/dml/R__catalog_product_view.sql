create or replace view catalog_product as select product.*,
    ...
    discount.discount_source as discount_source,
    discount.id as discount_id,
    (case when discount.id is null then false else true end) as discount_applied
from product
    left join lateral (
        select discount.id, campaign.description as discount_source, campaign_id, product_id, discount_type, discount
        from discount
            right join campaign
        ...
        where discount.product_id = product.id and
            discount.active = true and
            discount.claim_rule_id is null and
            discount.coupon_code is null
